...
public class LoginController extends BaseController {
	...
	public ModelAndView login(HttpServletRequest req){
		if(!BaseUtil.isEmpty(getSessionUser(req))){
			req.getSession().removeAttribute("user");
			req.getSession().removeAttribute("roleMap");
		}
    ...
    public AjaxJson mobileLogin(HttpServletRequest req, HttpServletResponse resp, 
    	...
    	try {
    		...
    		if(BaseUtil.isEmpty(tokenList) || BusinessUtil.isTokenInvalid(tokenList.get(0))){
...
//				req.getSession().setAttribute("menus", ModuleList);
		        ...
		        //如果是超管，则设置
		        userInfo.setIsSuperAdmin(isSuperAdmin(userInfo)?YesOrNo.是.getValue():YesOrNo.否.getValue());
		        //将用户存入session中
