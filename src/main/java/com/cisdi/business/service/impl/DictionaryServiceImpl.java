		...
		if(BaseUtil.isEmpty(id)){
			...
			if(BaseUtil.isEmpty(entity.getDicType())){
				...
				if(BaseUtil.isEmpty(parent))
					return AjaxJson.fail("未找到对应的父字典");
				//而后验证是否名称重复
				...
				if(!BaseUtil.isEmpty(dictionaryDao.selectByExample(dictionaryExample)))
					return AjaxJson.fail("名称不可重复");
				entity.setDicType(parent.getDicType());
			}else{
				//查询是否已有根字典
				DictionaryExample dictionaryExample = new DictionaryExample();
				dictionaryExample.or().andDicTypeEqualTo(entity.getDicType())
										.andDeleteflagEqualTo(DeleteFlag.未删除.getValue());
				if(!BaseUtil.isEmpty(dictionaryDao.selectByExample(dictionaryExample)))
					return AjaxJson.fail("不可重复添加");
			}
